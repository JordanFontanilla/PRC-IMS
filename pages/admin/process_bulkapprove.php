    ...
    try {
        ...
        foreach ($request_ids as $breq_token) {
            ...
            $update_borrow_request = $conn->prepare("
                ...
                SET breq_decisiondate = ?, breq_status = '4'  -- Status set to '4' (approved)
                WHERE breq_token = ?");
            ...
            // Update the is_approved column in tbl_borrow_request_items (set to 1)
            $update_is_approved = $conn->prepare("
                UPDATE tbl_borrow_request_items 
                SET is_approved = 1 
                WHERE breq_token = ?");
            $update_is_approved->bind_param("s", $breq_token);
